...
class AllApis {
  ...
  Future<dynamic> getProductsInRange(
    ...
    try {
      ...
      print('$base_url/products/getAllProducts?km=$km&latitude=$lat&longitude=$lon&city=$city&searchPurelyOnLocation=$searchPurelyOnLocation');
  ...
  Future<dynamic> changeProductState(
      {@required token, @required productId, @required state, @required storeId}) async {
    try {
      ...
      final body = json.encode({"productId": productId, "state": state, "store": storeId});
      http.Response response =
          await http.post(uri, headers: header, body: body);
      if (response.statusCode == 500) {
        navigate();
